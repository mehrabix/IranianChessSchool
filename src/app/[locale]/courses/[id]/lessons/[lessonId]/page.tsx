...
export default async function LessonPage(props: { params: Promise<{ id: string; lessonId: string }> }) {
  ...
  if (!lesson) return <div className="min-h-screen flex items-center justify-center"><p className="text-muted-foreground">{t('courses.lessonNotFound')}</p></div>;
      ...
      <Container size="lg">
        <div className="mb-6">
          <Button variant="ghost" size="sm" render={<Link href={`/courses/${courseId}`} />}>
            <ArrowLeft className="h-4 w-4 me-1 rtl:rotate-180" /> {t('courses.backToCourse')}
          </Button>
        </div>
        <div className="grid gap-8 lg:grid-cols-[1fr_300px]">
          <div>
            <div className="mb-6">
              <p className="text-sm text-muted-foreground mb-1">{mod ? getText(mod.title) : ''}</p>
              <h1 className="text-2xl font-bold">{getText(lesson.title)}</h1>
            </div>
            ...
            <div className="flex items-center justify-between mt-8">
              {prevLesson ? (
                <Button variant="outline" render={<Link href={`/courses/${courseId}/lessons/${prevLesson.id}`} />}>
                  <ArrowLeft className="h-4 w-4 me-1 rtl:rotate-180" /> {getText(prevLesson.title)}
                </Button>
              ) : <div />}
              {nextLesson ? (
                <Button render={<Link href={`/courses/${courseId}/lessons/${nextLesson.id}`} />}>
                  {getText(nextLesson.title)} <ArrowRight className="h-4 w-4 ms-1 rtl:rotate-180" />
                </Button>
              ) : <div />}
            </div>
          </div>
