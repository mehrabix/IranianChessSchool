    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-2 font-bold text-lg sm:text-xl shrink-0">
          <ChessKnight className="h-5 w-5 sm:h-6 sm:w-6 text-primary" />
        ...
        <nav className="hidden md:flex items-center gap-0.5 flex-1 justify-center overflow-x-auto">
          {navLinks.map((link) => (
            <Link
              ...
              href={link.href}
              className={cn(
                'flex items-center gap-1 md:gap-1.5 px-1.5 md:px-2 lg:px-3 py-2 text-xs md:text-sm font-medium rounded-md transition-colors whitespace-nowrap',
                isActive(link.href)
                  ? 'text-foreground bg-accent'
                  : 'text-muted-foreground hover:text-foreground hover:bg-accent/50'
              )}
            >
              <link.icon className="h-3.5 w-3.5 md:h-4 md:w-4 shrink-0" />
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            <SheetContent side={isRtl ? 'left' : 'right'} className="w-[280px] sm:w-[320px]">
              <SheetHeader className="mb-6">
                <SheetTitle className="flex items-center gap-2"><ChessKnight className="h-5 w-5 text-primary" />ICS</SheetTitle>
              </SheetHeader>
              <nav className="flex flex-col gap-1">
                {navLinks.map((link) => (
                  ...
                  ><link.icon className="h-5 w-5 shrink-0" />{t(link.labelKey)}</Link>
                ))}
              </nav>
              {session && (<>
