...
export function EngineEval({ fen, onBestMove }: EngineEvalProps) {
  ...
  if (error) return <p className="text-sm text-red-500">Engine error: {error}</p>;
      ...
      <CardContent className="space-y-3">
        ...
        {evalResult && (
          <div className="space-y-1 text-sm">
            <p><span className="text-muted-foreground">Score:</span> {evalResult.score > 0 ? '+' : ''}{evalResult.score.toFixed(2)}</p>
            <p><span className="text-muted-foreground">Depth:</span> {evalResult.depth}</p>
            <p><span className="text-muted-foreground">Best:</span> {evalResult.bestMove || 'N/A'}</p>
            <p className="text-muted-foreground truncate">
              <span className="text-muted-foreground">PV:</span> {evalResult.pv.join(' ')}
            </p>
          </div>
        )}
        {topLines.length > 0 && (
          <div className="space-y-1 border-t pt-2">
            <p className="text-xs font-medium text-muted-foreground">Top Lines</p>
            {topLines.map((line, i) => (
