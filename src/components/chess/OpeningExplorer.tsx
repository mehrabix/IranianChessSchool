...
export function OpeningExplorer({ fen, onMoveClick }: OpeningExplorerProps) {
  ...
  async function handleLoad() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Failed to load opening stats');
    } finally {
      setLoading(false);
    }
  }
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        {!loading && moves.length === 0 && !error && (
          <p className="text-xs text-muted-foreground">Click "Load Stats" to see opening statistics for this position.</p>
        )}
      </CardContent>
    </Card>
  );
