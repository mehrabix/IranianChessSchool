...
async function seed() {
  ...
  await db.insert(modules).values([
    { id: MOD_BOARD_ID, title: '_courses::chessFundamentals::modules::board', order: 1, courseId: BEGINNER_ID },
    { id: MOD_TACTICS_ID, title: '_courses::chessFundamentals::modules::tactics', order: 2, courseId: BEGINNER_ID },
  ]);
  ...
  await db.insert(modules).values([
    { id: MOD_POSITIONAL_ID, title: '_courses::intermediateStrategy::modules::positional', order: 1, courseId: INTERMEDIATE_ID },
  ]);
  ...
  console.log('Seed completed — i18n-keyed courses inserted');
}
